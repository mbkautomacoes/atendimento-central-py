...
export default function ChatPage({ params }: { params: { conversationId: string } }) {
  ...
  async function init() {
    ...
    channelRef.current = supabase
      .channel(`lead-chat-${conversationId}`)
      .on('postgres_changes', {
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0b141a]">
      ...
      <div className="flex items-center gap-3 px-4 py-3 bg-[#202c33] shadow">
        ...
        <div>
          <p className="text-[#e9edef] font-medium">{conversation?.lead_name}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <p className="text-[#8696a0] text-sm">Nenhuma mensagem ainda. Diga olá!</p>
          </div>
        ) : messages.map(msg => (
          <div key={msg.id} className={`flex ${msg.sender_type === 'lead' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[65%] rounded-lg px-3 py-2 shadow-md ${msg.sender_type === 'lead' ? 'bg-[#005c4b]' : 'bg-[#202c33]'}`}>
